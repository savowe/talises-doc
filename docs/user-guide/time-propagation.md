...
```xml
  ...
  <FILENAME_2>0.000_2.bin</FILENAME_2>
```
We specify the file paths of the wave-functions generated by gen_psi_0.
We load the files as the two internal states of which one is the Gaussian wave-packet and the other one empty.  
...
The mass of the particle is \\(m=5\cdot 10^{-25} \,\text{kg}\\).  
...
every 20th step. The wave-packet freely propagates.
...
\end{pmatrix}.
$$
The Rabi-Hamiltonian in Dirac-picture and Rabi-frequency \\(\Omega_R=2\pi f_R = 2\pi \cdot 2.5 \,\text{MHz}\\), as defined in the `<CONSTANTS>` item.
